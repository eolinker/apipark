 ...
 * @LastEditTime: 2024-06-04 19:03:23
 * @FilePath: \frontend\packages\core\src\pages\systemRunning\SystemRunning3.tsx
 */
...
import { PictureTypeEnum, NodeClickItem, GraphData } from "../../const/system-running/type.ts";
import { UnionFind, edgesFormatter, getNodeSpacing, nodesFormatter } from "@common/utils/systemRunning.ts";
import { EDGE_STYLE, END_ARROW_STYLE, OUT_SPACE_CONTENT_EDGE_COLOR, RELATIVE_PICTURE_NODE_FONTSIZE, SELF_SPACE_CONTENT_EDGE_COLOR, SYSTEM_TUNNING_CONFIG } from "../../const/system-running/const.ts";
...
export default function SystemRunning(){
   ...
   */
   ...
   // 从 edges 中提取所有唯一的节点，并将 Set 转换为数组
    ...
    const getNodeData = ()=>{
      setLoading(true)
        fetchData<BasicResponse<TopologyItem>>('topology',{method:'GET',eoParams:{id:partitionId},eoTransformKeys:['invoke_services']},).then(response=>{
    ...
    useEffect(() => {
      ...
      return (<>
        ...
        :<Spin className="h-[calc(100vh-50px)]" indicator={<LoadingOutlined style={{ fontSize: 24 }} spin/>} spinning={loading}>{!loading && <SystemRunningInstruction />}</Spin>
              }</>)
    }
