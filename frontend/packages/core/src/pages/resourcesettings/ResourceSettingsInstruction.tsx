...
export default function ResourceSettingsInstruction() {
    ...
    return (
        ...
        <p className="text-[20px] font-medium leading-[32px] text-MAIN_TEXT">环境配置并开启资源插件</p>
        <p className="text-[12px] font-normal leading-[20px] text-DESC_TEXT mt-[12px]" >资源插件用于增强网关的功能和性能。在启用资源类插件之前，请确保已经配置了环境。例如，Redis插件可以提高缓存和速率限制的性能，配置完成后，可以使用Redis作为缓存数据库。</p>
        ...
        <div className="flex mt-[28px]">
            ...
            <p className="text-[20px] font-medium leading-[32px] text-MAIN_TEXT">环境配置</p>
                        <p className="text-[12px] font-normal leading-[20px] text-DESC_TEXT">新增环境的名称、描述和其他相关属性，并设置该环境内的集群地址，以确保插件能够正确识别和连接到这些集群</p>
                        <p><Link to="/partition/list">添加环境/配置已有环境的集群地址</Link></p>
            </div>
        </div>
    </div></div>
    )
