...
func (cfg *Config) Detail() []project_authorization_dto.DetailItem {
	...
	items := []project_authorization_dto.DetailItem{
	...
	switch cfg.Algorithm {
	case "HS256", "HS384", "HS512":
		items = append(items, project_authorization_dto.DetailItem{Key: "Secret", Value: cfg.Secret})
		base64 := "false"
		if cfg.SignatureIsBase64 {
			base64 = "true"
		}
		items = append(items, project_authorization_dto.DetailItem{Key: "Secret", Value: base64})
	default:
		items = append(items, project_authorization_dto.DetailItem{Key: "RSA公钥", Value: cfg.PublicKey})
	}
