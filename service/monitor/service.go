...
type IMonitorService interface {
	...
	// MapByPartition 获取监控配置
	MapByPartition(ctx context.Context, partitionId ...string) (map[string]*Monitor, error)
	GetByPartition(ctx context.Context, partitionId string) (*Monitor, error)
	// Save 保存监控配置
	Save(ctx context.Context, monitor *SaveMonitor) error
	DeleteByPartition(ctx context.Context, partitionId string) error
}
